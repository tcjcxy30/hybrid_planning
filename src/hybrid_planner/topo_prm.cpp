...
void TopologyPRM::init(double resolution)
{
    graph_.clear();
    discretize_num_ = 20;
    ...
    // offset_ =
    //     Eigen::Vector3d(0.5, 0.5, 0.5) - edt_environment_->sdf_map_->getOrigin() / resolution_;
...
void TopologyPRM::findTopoPaths(nav_msgs::msg::OccupancyGrid& globalMap, Eigen::Vector3d start,
                                Eigen::Vector3d end, list<GraphNode::Ptr>& graph,
                                ...
                                vector<vector<Eigen::Vector3d>>& select_paths)
{
    ...
    // collision checker initialization
    collision_checker_ = std::make_shared<CollisionChecker>(globalMap);

    // start_pts_ = start_pts;
    // end_pts_   = end_pts;

    // 1 generate prm note graph
    ...
    // 3. use std::thread to shortcut all candidate paths parallel
    shortcutPaths();
    ...
    // 4. delete the path with the same homotopy
    ...
    // 5. seletc the short path in each homotopy
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end)
{
    ...
    while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
        ...
        // 判断该点有没有碰撞，直接读图即可
        if (!collision_checker_->isValid(pt(0), pt(1))) {
            continue;
        }
        ...
        // 1.1 check how much guard can be seen by current point
        /* find visible guard */
        ...
        if (visib_guards.size() == 0) {
            ...
            // 1.2 check whether guard1-pt-guard2 and guard1-guard2 are the same homotopy
            // 这个操作可能影响到最终路径是更短而不是更符合需求。比如一个拐弯可能被拉成一条直线。
            // 有点奇怪，三个点的距离怎么可能小于两点直连？
            // TODO：这里加一个判断，需要限制两个可见点之间的距离，不能大于多少?
    ...
    // 1.3 delete the guard only have one neighbor. A connector has at least two neighbors.
    pruneGraph();
...
vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt)
{
    ...
    for (list<GraphNode::Ptr>::iterator iter = graph_.begin(); iter != graph_.end(); ++iter) {
        ...
        // if two point is so far, continue


        // 1.1.1.1 check if the line between two guard is collision
    ...
    for (int i = 0; i < g1->neighbors_.size(); ++i) {
        for (int j = 0; j < g2->neighbors_.size(); ++j) {
            if (g1->neighbors_[i]->id_ == g2->neighbors_[j]->id_) {
                ...
                // also check by line collision checking
                // 貌似true 是不同伦
                            ...
                            Eigen::Vector3d& pc, int caster_id)
{
    ...
    // check if each point in line has collision, respectively
    casters_[caster_id].setInput(p1, p2);
    while (casters_[caster_id].step(ray_pt)) {
        bool is_vaild = collision_checker_->isValid(ray_pt(0), ray_pt(1));
        if (!is_vaild) {
            return false;
        }
    }
...
void TopologyPRM::pruneGraph()
{
    ...
    if (graph_.size() > 2) {
        for (list<GraphNode::Ptr>::iterator iter1 = graph_.begin();
             iter1 != graph_.end() && graph_.size() > 2;
             ++iter1) {
...
void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num)
{
    ...
    //     // discretize the path, only need to do this
    ...
    //     // I don't need to shortcut the path
    ...
    // discretize the path, only need to do this
    // 按照指定点数离散，还是按照分辨率来考虑点数再离散？
    // 前者时间比较一致，后者路径的步长比较一致
    vector<Eigen::Vector3d> dis_path = discretizePath(short_path, discretize_num_);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "path num: %d", dis_path.size());
    short_paths_[path_id] = dis_path;
}
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(vector<Eigen::Vector3d> path)
{
    ...
    if (path.size() < 2) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "what path? ");
        return dis_path;
    }
...
vector<vector<Eigen::Vector3d>> TopologyPRM::searchPaths()
{
    ...
    // find all feasible path, and sort by number of guards
